...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('Nanz Botz', {
		font: 'Standard',
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `NanzBot bot`, img,but = [{buttonId:`apaaja`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `NanzBot bot`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `AMIN`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
